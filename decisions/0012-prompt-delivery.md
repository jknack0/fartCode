...
- **`fartcode-core::pty`** — `build_command(ctx, provider) -> AgentCommand`
...
- **`PromptDescriptor` extended** (fartcode-providers) with the reference spec
...
- **Spill** (ticket addition, not in the reference): prompts ≥ 32KB spill to
  `<worktree>/.fartCode/prompts/<uuid>.md`, passed as `@<path>`; `build_command_with_spill`
