...
contract (`ADE_*`), status events, output tail, and session dedupe.
ARCHITECTURE §7 wires `pty_manager: Arc<dyn ade_terminal::PtyManager>`
wired in `ade-app`, implying the trait lives in `ade-terminal`. But the
lifecycle service lives in `ade-core` (ticket crate), and `ade-terminal`
depends on `ade-core` — putting the trait in `ade-terminal` would create a
dependency cycle.

## Decision

1. **Trait in `ade-core`**: `ade_core::terminals::pty::{PtyManager, PtyHandle}`
   — the same pattern as `GitOps` (trait in `ade-core`, impl in `ade-git`).
   `ade-terminal` depends on `ade-core` + `portable-pty` and provides
   `PortablePtyManager`. §7's field type becomes
   `Arc<dyn ade_core::terminals::pty::PtyManager>`; wiring location unchanged.
...
4. **Env contract is ade-specific** (not in the emdash reference): the ticket
   defines `ADE_TASK_ID`, `ADE_TASK_NAME` (slugified via
   `tasks::naming::sanitize_name`, fallback `task`), `ADE_TASK_PATH`,
   `ADE_ROOT_PATH`, `ADE_DEFAULT_BRANCH` (default `main`), and
   `ADE_PORT = 50000 + (hash32(portSeed) % 1000) * 10` (FNV-1a 32, no extra
   deps). Port seed = workspace path (fallback task id) so two tasks in the
   same project get different ports (acceptance 2).

## Consequences

- `ade-core` tests can exercise the full PTY path via a dev-dependency on
  `ade-terminal` (dev-dep cycles are allowed, unlike regular deps).
