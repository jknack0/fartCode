...
1. **One module, one file** (`fartcode_core::pty::env_allowlist`): the canonical
   ...
   (`FARTCODE_HOOK_*`). Adding a var = touching one list + PR review; the
...
2. **`build_agent_env`** (reference `pty-env.ts::buildAgentEnv`): base env →
   ...
   registry `env_vars`) → task env (`FARTCODE_*` from E1-06) → hook env.
