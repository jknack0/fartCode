...
Opening a project must leave the repo ready to task: ade internals git-excluded,
...
- `.ade/` is excluded from git via **`.git/info/exclude`** (local, per-repo —
...
- `open_project` = seed settings → exclude `.ade/` → re-detect worktrees via
...
- Repos stay clean of `.ade/` without touching tracked files.
