...
use fartcode_acp::transcript::models::{
    PlanEntryStatus, Role, ThinkingStatus, ToolItemKind, ToolStatus, TranscriptItem,
    TranscriptTurnOutcome,
};
use fartcode_acp::transcript::{MessageRole, NormalizedEvent, TranscriptParser};
use serde_json::json;
...
fn interleaved_sequence_produces_the_exact_transcript() {
    ...
    assert_eq!(
        turn.initiator,
        fartcode_acp::transcript::TurnInitiator::User
    );
    assert_eq!(
        turn.outcome,
...
fn agent_content_without_user_message_lazily_opens_an_agent_turn() {
    ...
    assert_eq!(
        active.initiator,
        fartcode_acp::transcript::TurnInitiator::Agent
    );
