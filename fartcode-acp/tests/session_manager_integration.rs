...
use fartcode_acp::session::{SessionIdStore, SessionManager, StartInput};
use fartcode_acp::{Error, Lifecycle, QueuedPrompt};
...
fn prompt_text(turn: &fartcode_acp::transcript::TranscriptTurn) -> &str {
    turn.items
        .iter()
        .find_map(|item| match item {
            fartcode_acp::transcript::TranscriptItem::Message(m)
                if m.role == fartcode_acp::transcript::Role::User =>
            {
                Some(m.text.as_str())
            }
            _ => None,
...
fn agent_text(turn: &fartcode_acp::transcript::TranscriptTurn) -> String {
    turn.items
        .iter()
        .filter_map(|item| match item {
            fartcode_acp::transcript::TranscriptItem::Message(m)
                if m.role == fartcode_acp::transcript::Role::Assistant =>
            {
                Some(m.text.clone())
            }
            _ => None,
...
async fn queue_ops_while_busy_then_ordered_drain_after_cancel() {
    ...
    assert_eq!(
        ...
        Some(fartcode_acp::transcript::TranscriptTurnOutcome::Cancelled { reason: None })
    );
...
async fn permission_round_trip_through_cell() {
    ...
    let tool_done = turn.items.iter().any(|item| match item {
        fartcode_acp::transcript::TranscriptItem::Tool(t) => {
            t.tool_call_id == "call_001" && t.status == fartcode_acp::transcript::ToolStatus::Done
        }
        _ => false,
    });
