...
fn conversation_dto(conv: &fartcode_core::conversations::model::Conversation) -> ConversationDto {
    ConversationDto {
        id: conv.id.clone(),
...
pub fn create_conversation(
    ...
    let acp_capable = provider
        .as_deref()
        .and_then(fartcode_providers::get)
    ...
    app.conversations
        ...
        .map(|convs| {
            convs
                .iter()
                .filter(|c| {
                    c.scope == fartcode_core::conversations::model::ConversationScope::Project
                })
...
pub fn get_or_create_project_conversation(
    ...
    let acp_capable = fartcode_providers::get(&provider).is_some_and(|p| p.capabilities.acp);
    if !acp_capable {
        return Err(format!(
    ...
    let existing = app
        ...
        .find(|c| {
            c.scope == fartcode_core::conversations::model::ConversationScope::Project
                && c.config.r#type == fartcode_core::conversations::model::ConversationType::Acp
        });
...
pub fn acp_history(
    runtime: State<'_, Arc<AcpRuntime>>,
    conversation_id: String,
) -> Result<Option<fartcode_acp::LiveModels>, String> {
