...
fn terminal_session_survives_attach_client_death_and_reattaches() {
    ...
    write_retry(
        ...
        &format!(
            ...
            fartcode_core::shell_escape::single_quote(&marker.to_string_lossy())
        ),
    );
    ...
    write_retry(
        ...
        &format!(
            "pwd > {}\n",
            fartcode_core::shell_escape::single_quote(&proof.to_string_lossy())
        ),
    );
    let deadline = Instant::now() + Duration::from_secs(15);
...
fn sweep_never_touches_foreign_sessions() {
    ...
    // Positive control: an fartCode session matching the prefix MUST die.
    let ours = make_tmux_session_name(&format!("{prefix}0"));
    // Foreign: a NON-fartCode-named live session must never be matched — even
    // though its name embeds the prefix text.
    let foreign = format!("fartCode-sweepcheck-{pid}-lookalike");
