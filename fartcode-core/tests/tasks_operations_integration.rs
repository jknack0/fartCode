...
struct Fixture {
    ...
    project: fartcode_core::projects::model::Project,
    ...
    fn params(&self) -> CreateTaskParams {
        CreateTaskParams {
            ...
            git: GitSetup::CreateBranch {
                branch_name: "fartCode/fix-the-bug-ab12c".into(),
...
fn create_with_new_worktree_provisions_rows_worktree_and_events() {
    ...
    assert_eq!(
        ...
        Some("fartCode/fix-the-bug-ab12c")
    );
...
fn provision_is_idempotent_and_reads_intent_from_config() {
    ...
    assert!(first
        ...
        .as_deref()
        .unwrap()
        .ends_with("fartCode/fix-the-bug-ab12c"));
    assert!(wt_path.exists());
...
fn create_use_branch_checks_out_existing_branch() {
    ...
    fx.git
        .branch_create(repo, "fartCode/existing-fix", "main")
        .unwrap();

    let mut params = fx.params();
    params.git = GitSetup::UseBranch {
        branch_name: "fartCode/existing-fix".into(),
    };
...
fn provision_heals_legacy_configless_worktree_row() {
    ...
    let store = fartcode_core::tasks::DbTaskStore::new(fx.db.clone(), fx.bus.clone());
    let task = fartcode_core::tasks::TaskStore::create(
        &store,
        fartcode_core::tasks::CreateTaskOptions {
    ...
    assert!(
        config.contains("fartCode/legacy-task"),
    ...
    assert!(
        branches
            .iter()
            .any(|b| b.name.starts_with("fartCode/legacy-task-")),
