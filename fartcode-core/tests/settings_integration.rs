...
impl Fixture {
    ...
    fn fartcode_json(&self) -> PathBuf {
        self.repo.join(PROJECT_CONFIG_FILE)
    }

    fn write_ade_json(&self, content: &str) {
        std::fs::write(self.fartcode_json(), content).unwrap();
    }
}
...
fn test_local_project_paths_default_to_home() {
    ...
    assert!(
        local
            ...
            .ends_with("fartCode/worktrees"),
        "worktree dir must default under ~/fartCode: {}",
        local.default_worktree_directory
    );
}

// ---------------------------------------------------------------------------
// Acceptance 2: .fartCode.json honored; local value overrides it; clearing local
...
fn test_ade_json_precedence_and_local_override() {
    ...
    assert_eq!(
        ...
        ".fartCode.json preservePatterns must be honored"
    );
    ...
    assert_eq!(
        ...
        "local value must override .fartCode.json"
    );
    ...
    assert_eq!(
        settings.preserve_patterns.as_deref(),
        Some(&[".env".to_string(), "custom.env".to_string()][..]),
        "clearing the local value must fall back to .fartCode.json"
    );
}
...
fn test_unparseable_ade_json_falls_back() {
    ...
    assert_eq!(
        patterns,
        fartcode_core::settings::DEFAULT_PRESERVE_PATTERNS.to_vec(),
        "unparseable .fartCode.json must fall back to defaults"
    );
}
...
fn test_task_startup_command_round_trip_and_not_shareable() {
    ...
    // forces it), taskStartupCommand must not leak into .fartCode.json.
    let local = ProjectSettings {
...
fn test_ade_json_filtered_from_preserve_patterns() {
    let fx = Fixture::new();
    fx.write_ade_json(r#"{"preservePatterns": [".env", ".fartCode.json", "keep.txt"]}"#);
    let settings = fx
        .settings
        .get_project_settings(&fx.project_id, &fx.repo)
        .unwrap();
    let patterns = settings.preserve_patterns.unwrap();
    assert!(
        !patterns.iter().any(|p| p == ".fartCode.json"),
...
fn test_legacy_migration_runs_once() {
    ...
    assert_eq!(
        ...
        Some(fartcode_core::settings::DefaultBranch::Name(
            "upstream/main".into()
        )),
...
fn test_seed_default_preserve_patterns_when_no_file() {
    ...
    assert_eq!(
        ...
        serde_json::json!(fartcode_core::settings::DEFAULT_PRESERVE_PATTERNS)
    );
}
...
fn test_share_with_team_moves_values_to_file() {
    ...
    let content = std::fs::read_to_string(fx.fartcode_json()).unwrap();
    assert!(
        content.ends_with('\n'),
        ".fartCode.json must end with a newline"
    );
...
fn test_share_with_team_preserves_unparseable_ade_json() {
    ...
    assert!(
        ...
        "unparseable .fartCode.json must abort sharing: {err}"
    );
    let content = std::fs::read_to_string(fx.fartcode_json()).unwrap();
...
fn test_invalid_worktree_directory_falls_back_on_read() {
    ...
    let settings = ProjectSettings {
        worktree_directory: Some("~/fartCode/wt".into()),
        ..Default::default()
    };
    fx.settings
        .update_project_settings(&fx.project_id, &fx.repo, &settings)
...
fn test_normalize_worktree_directory_rules() {
    use fartcode_core::settings::worktree_directory::normalize_worktree_directory;
    ...
    assert!(matches!(
        ...
        Err(fartcode_core::Error::InvalidWorktreeDirectory(_))
    ));
    assert!(matches!(
...
// ---------------------------------------------------------------------------
// E1-05: update stores only the delta vs .fartCode.json (file keeps precedence)
// ---------------------------------------------------------------------------
