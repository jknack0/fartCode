...
//! untracked matches appear in new worktrees, tracked files and `.fartCode.json`
//! never do, and repo `.fartCode.json` patterns override project defaults.
...
impl Fixture {
    ...
    /// a repo that already carries `.fartCode.json` when added gets the file's
...
/// Default patterns copy untracked matches; tracked files and `.fartCode.json` never
/// reach the worktree (acceptance: ".env/.env.local/docker-compose.override.yml
/// appear; tracked files and .fartCode.json never copied").
#[test]
fn preserve_copy_defaults_skip_tracked_and_ade_json() {
    ...
    .unwrap();
    std::fs::write(
        fx.repo.join(".fartCode.json"),
        r#"{"preservePatterns": [".env", ".env.local"]}"#,
    )
    .unwrap();

    let wt = fx.create_task_with_worktree("fartCode/preserve-1");
    ...
    assert!(
        !wt.join(".fartCode.json").exists(),
        ".fartCode.json never copied"
    );
}

/// Repo `.fartCode.json` patterns override project defaults (acceptance: "patterns
/// from task `.fartCode.json` override project defaults"). The file must exist when
...
fn preserve_copy_ade_json_patterns_override_defaults() {
    ...
    let wt = fx.create_task_with_worktree("fartCode/preserve-2");
    ...
    assert!(
        ...
        ".env.local must NOT be copied once .fartCode.json overrides the defaults"
    );
    assert!(
        !wt.join("docker-compose.override.yml").exists(),
...
fn fixture_provisions_a_worktree() {
    let fx = Fixture::new();
    let wt = fx.create_task_with_worktree("fartCode/preserve-3");
...
fn remove_worktree_refuses_dirty_worktree() {
    let fx = Fixture::new();
    let wt = fx.create_task_with_worktree("fartCode/dirty-test");
    ...
    let (task_id, workspace_id): (String, String) = fx
        ...
        .unwrap();
    let project = fartcode_core::projects::model::Project {
        id: fx.project_id.clone(),
        name: "demo".into(),
        path: fx.repo.clone(),
        workspace_provider: fartcode_core::projects::model::WorkspaceProviderKind::Local,
        base_ref: None,
    ...
    let wm = fartcode_core::projects::worktrees::WorktreeManager::new(
        fx.db.clone(),
        fx.settings.clone(),
    ...
    let err = wm
        ...
        .unwrap_err();
    assert!(
        matches!(err, fartcode_core::Error::DirtyWorktree(_)),
        "{err:?}"
    );
...
fn delete_project_cascades_rows_and_tears_down_worktrees() {
    let fx = Fixture::new();
    let wt = fx.create_task_with_worktree("fartCode/delete-me");
    ...
    // Worktree pool dir torn down from disk. wt = <pool>/fartCode/<branch>, so the
