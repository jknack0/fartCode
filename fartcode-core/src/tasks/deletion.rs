...
impl TaskDeletionService {
    ...
    fn teardown_sessions(&self, project_id: &str, task_id: &str) {
        ...
        for leaf in leaf_ids {
            ...
            // (survives an fartCode kill). Best-effort — absence is Ok.
    ...
    fn provisioned_branch_follows_config_then_column() {
        ...
        let ws = WorkspaceSnapshot {
            ...
            config: Some(serde_json::json!({
                "version": "2",
                "git": { "kind": "create-branch", "branchName": "fartCode/task",
                         "fromBranch": { "type": "local", "branch": "main" } },
                "workspace": { "kind": "new-worktree" }
            })),
        };
        assert_eq!(provisioned_branch(&ws).as_deref(), Some("fartCode/task"));

        // No config → the recorded column.
        let mut ws_col = ws.clone();
        ws_col.config = None;
        ws_col.branch_name = Some("fartCode/col".into());
        assert_eq!(provisioned_branch(&ws_col).as_deref(), Some("fartCode/col"));
