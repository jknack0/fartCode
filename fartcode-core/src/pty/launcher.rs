...
//! `fartcode_core::terminals::pty::PtyManager`; the interactive-terminal wiring
...
use fartcode_providers::{PromptStrategy, ProviderDescriptor};
...
pub struct AgentLaunchContext {
    ...
    /// `FARTCODE_*` task env (E1-06 contract) — overlays the allowlist.
...
impl AgentLauncher {
    ...
    pub fn resolve_binary(&self, provider_id: &str) -> Result<PathBuf, Error> {
        ...
        if let Some(provider) = fartcode_providers::get(provider_id) {
    ...
    pub fn run(&self, ctx: &AgentLaunchContext) -> Result<AgentLaunchOutcome, Error> {
        let provider = fartcode_providers::get(&ctx.provider_id)
