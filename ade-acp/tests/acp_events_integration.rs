...
use ade_acp::transcript::models::{ThinkingStatus, ToolItemKind, ToolStatus};
...
async fn live_models_snapshot_reduces_the_rich_turn() {
    ...
    match &items[0] {
        ade_acp::transcript::TranscriptItem::Message(m) => {
            assert_eq!(m.role, ade_acp::transcript::Role::User);
    ...
    let thinking = items.iter().find_map(|i| match i {
        ade_acp::transcript::TranscriptItem::Thinking(t) => Some(t),
        _ => None,
    });
    ...
    let tool = items.iter().find_map(|i| match i {
        ade_acp::transcript::TranscriptItem::Tool(t) if t.kind == ToolItemKind::ExecuteToolCall => {
            Some(t)
        }
        _ => None,
    });
    ...
    assert_eq!(
        ...
        ade_acp::transcript::PlanEntryStatus::InProgress
    );
...
async fn resumed_session_replay_settles_as_committed_turns() {
    ...
    let texts: Vec<&str> = turn
        ...
        .filter_map(|i| match i {
            ade_acp::transcript::TranscriptItem::Message(m) => Some(m.text.as_str()),
            _ => None,
        })
        .collect();
