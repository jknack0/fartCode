...
use ade_acp::{AcpClient, Error, PermissionDecision, PermissionRequest};
...
fn agent_text(updates: &[ade_acp::SessionUpdateEvent]) -> String {
    updates
        .iter()
...
fn tool_call_statuses(updates: &[ade_acp::SessionUpdateEvent]) -> Vec<&'static str> {
    updates
        .iter()
        .filter_map(|u| match &u.update {
