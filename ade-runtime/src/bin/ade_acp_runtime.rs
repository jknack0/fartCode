//! `ade-acp-runtime` — the out-of-process ACP worker (E2-11-2).
//!
//! Hosted by `ade-app` via `ade_runtime::SessionHost`. One worker = one ACP
...
//! The renderer never contributes env (see `ade_runtime::protocol`).

use std::collections::HashMap;
use std::sync::Arc;

use ade_acp::{AcpClient, PermissionDecision, PermissionRequest, StdioTransport};
use ade_runtime::process_host::{AdapterProcessConfig, LocalProcessHost, ProcessHost};
use ade_runtime::protocol::{worker_methods, WorkerSession};
...
async fn start_session(
...
) -> Result<(Arc<AcpClient>, agent_client_protocol_schema::v1::SessionId), ade_runtime::Error> {
    // Spawn the adapter through the process host (env = server-resolved
    // credentials only; see ade_runtime::protocol).
    ...
    let transport = Arc::new(
        StdioTransport::from_child(child, updates_tx, events_tx)
            .map_err(|e| ade_runtime::Error::Protocol(e.to_string()))?,
    );
    ...
    let resolver = move |request: PermissionRequest, answerer: Arc<ade_acp::PermissionAnswerer>| {
