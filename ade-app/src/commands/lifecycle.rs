...
//! runs via `sh -c '<script>'` in the task worktree with the ADE_* env
...
use ade_core::settings::{DefaultBranch, ProjectSettings};
use ade_core::tasks::TaskStore;
use ade_core::terminals::lifecycle::{task_env, LifecycleScriptType};
use tauri::State;
...
/// ADE_DEFAULT_BRANCH for the env contract: the project's configured
...
pub(crate) fn spawn_lifecycle_script<R: tauri::Runtime>(
    ...
    // project get isolated ADE_PORTs (two tasks never share a port).
    let env = task_env(
        task_id,
        &task.name,
        Path::new(&ctx.cwd),
...
/// setup/run/teardown in the worktree with the ADE_* env contract. Returns
