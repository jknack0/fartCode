...
use ade_core::settings::{ProjectSettings, SettingsStore};
use tauri::State;
...
pub fn update_project_settings(
    ...
    let settings = match &settings.worktree_directory {
        ...
        Some(wd) => {
            let normalized = ade_core::settings::worktree_directory::normalize_worktree_directory(
                wd,
                ade_core::settings::worktree_directory::home_dir().as_deref(),
            )
            .map_err(|e| e.to_string())?;
            ProjectSettings {
