...
entries = []
for f in sorted(glob.glob('ade-providers/extracted/t*/*.json')):
    d = json.load(open(f))
    c = d["capabilities"]
    cap = (
        "Capabilities {\n"
...
header = """//! GENERATED from `ade-providers/extracted/` (E3-01 registry data extraction).
...
"""
footer = "]);\n"
open('ade-providers/src/providers_data.rs', 'w').write(header + ",\n".join("    " + e for e in entries) + "\n" + footer)
