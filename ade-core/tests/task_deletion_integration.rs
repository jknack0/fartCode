...
use ade_core::tasks::deletion::{DeleteTaskOptions, TaskDeletionService};
use ade_core::tasks::operations::{
    CreateTaskParams, GitSetup, InitialConversationConfig, SourceBranchRef, TaskConfigParams,
    TaskCreationService,
};
use ade_core::tasks::{CreateTaskOptions, DbTaskStore, TaskStore, WorkspaceTarget};
...
fn delete_removes_worktree_rows_view_state_and_reaps_session() {
    let fx = Fixture::new();
    let (task_id, worktree) = fx.create_provisioned("ship it", "ade/ship-it");
...
fn delete_branch_only_when_created_from_different_branch() {
    let fx = Fixture::new();
    let (task_id, _) = fx.create_provisioned("branching", "ade/branching");
    assert!(fx.branch_exists("ade/branching"));
    ...
    fx.deletion
        ...
        .unwrap();
    assert!(
        fx.branch_exists("ade/branching"),
        "branch survives by default"
    );

    // deleteBranch=true: the created branch differs from its source → gone.
    let (task_id2, _) = fx.create_provisioned("branching 2", "ade/branching-2");
    fx.deletion
        .delete_task(
            &fx.project_id,
            &task_id2,
        ...
        .unwrap();
    assert!(
        !fx.branch_exists("ade/branching-2"),
...
fn sibling_task_keeps_worktree_but_row_is_deleted() {
    let fx = Fixture::new();
    let (task_a, worktree) = fx.create_provisioned("first", "ade/shared");
...
fn double_delete_is_idempotent() {
    let fx = Fixture::new();
    let (task_id, worktree) = fx.create_provisioned("once", "ade/once");
...
fn delete_during_provision_is_safe() {
    ...
    let created = fx
        .creation
        .create(fx.params("racing", "ade/racing"))
        .unwrap();
