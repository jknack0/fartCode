...
use ade_core::terminals::lifecycle::{
    ade_port, append_output_tail, create_lifecycle_script_terminal_id, hash32,
    ...
    LifecycleScriptType, RunOptions, ADE_PORT_BASE, OUTPUT_TAIL_CAP,
};
...
#[test]
fn ade_port_is_deterministic_and_isolated() {
    // Range: 50000..=59990, step 10 (ticket formula).
    for seed in ["", "a", "hello world", "/some/path"] {
        let p = ade_port(seed);
        assert!((ADE_PORT_BASE..=ADE_PORT_BASE + 9990).contains(&p), "{p}");
        assert_eq!((p - ADE_PORT_BASE) % 10, 0, "step 10: {p}");
        assert_eq!(ade_port(seed), ade_port(seed), "deterministic");
    }
    // Port isolation: two tasks in the same project have different
    // worktree paths → different ports (acceptance 2).
    let p1 = ade_port("/proj/worktrees/a");
    let p2 = ade_port("/proj/worktrees/b");
...
fn task_env_contract_is_exact() {
    ...
    assert_eq!(map.get("ADE_TASK_ID").unwrap(), "task-1");
    assert_eq!(
        map.get("ADE_TASK_NAME").unwrap(),
        "fix-the-bug",
        "slugified"
    );
    assert_eq!(map.get("ADE_TASK_PATH").unwrap(), "/wt/fix-the-bug");
    assert_eq!(map.get("ADE_ROOT_PATH").unwrap(), "/proj");
    assert_eq!(map.get("ADE_DEFAULT_BRANCH").unwrap(), "main");
    assert_eq!(
        map.get("ADE_PORT").unwrap().parse::<u16>().unwrap(),
        ade_port("/wt/fix-the-bug")
    );
    ...
    let map: std::collections::HashMap<_, _> = env.into_iter().collect();
    assert_eq!(map.get("ADE_TASK_NAME").unwrap(), "task");
    assert_eq!(
        map.get("ADE_DEFAULT_BRANCH").unwrap(),
        "main",
        "empty -> main fallback"
    );
}
...
fn setup_script_runs_with_env_and_emits_statuses() {
    ...
    let run = fx
        ...
        .run(
            &sid,
            &script(LifecycleScriptType::Setup, "echo $ADE_TASK_ID:$ADE_PORT"),
            fx.cwd.path(),
            &fx.env("task-42", "/wt/a"),
            &RunOptions {
                wait_for_exit: true,
        ...
        .unwrap();
    let out = env_of(&run);
    assert!(out.contains("task-42"), "ADE_TASK_ID in output: {out:?}");
    let port = ade_port("/wt/a").to_string();
    assert!(out.contains(&port), "ADE_PORT in output: {out:?}");
    assert_eq!(
        fx.events_for(&sid),
...
fn shell_arithmetic_on_ade_port_works() {
    let fx = Fixture::new();
    let sid = make_lifecycle_script_session_id("p", "w", LifecycleScriptType::Setup);
    let port = ade_port("/wt/arith");
    let run = fx
        .service
        .run(
            &sid,
            &script(LifecycleScriptType::Setup, "echo $((ADE_PORT + 1))"),
            fx.cwd.path(),
            &fx.env("t", "/wt/arith"),
            &RunOptions {
                wait_for_exit: true,
    ...
    assert!(
        ...
        "arithmetic sees ADE_PORT ({port}): {out:?}"
    );
}
...
fn failure_surfaces_unless_continue_on_failure() {
    ...
    assert!(
        matches!(
            err,
            ade_core::Error::LifecycleScriptFailed {
                exit_code: Some(3),
                ..
            }
        ),
...
fn timeout_stops_the_script() {
    ...
    let err = fx
        ...
        .unwrap_err();
    assert!(
        matches!(err, ade_core::Error::LifecycleScriptTimeout(_)),
        "{err:?}"
    );
