...
fn write_fake_amp(dir: &Path) -> PathBuf {
    ...
    std::fs::write(
        &script,
        "#!/bin/bash\necho \"args=$*\" > \"$ADE_TASK_PATH/amp-report.txt\"\nexit 0\n",
    )
    .unwrap();
...
impl Fixture {
    fn new() -> Self {
        ...
        // ADE_TASK_PATH must exist (task_env contract) for the fake's redirect.
    ...
    fn fresh_launch_ctx(&self, conversation_id: &str) -> AgentLaunchContext {
        AgentLaunchContext {
            ...
            task_env: vec![], // the fake writes via ADE_TASK_PATH (contract)
            hook_env: None,
