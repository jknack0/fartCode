...
use ade_core::pty::launcher::{AgentLaunchContext, AgentLauncher};
use ade_core::Error;
...
echo "ade_task=$ADE_TASK_ID ade_port=$ADE_PORT args=$* secret=${{SECRET_TOKEN:-absent}} term=$TERM" > "$GOOSE_REPORT"
exit {exit_code}
"#
        ),
    )
...
fn launches_agent_in_worktree_with_allowlisted_env_and_prompt() {
    ...
    assert!(report.contains("ade_task=task-1"), "task env: {report}");
    let port = report
        .split("ade_port=")
        .nth(1)
