...
//! - Path resolution: explicit `db_path` arg > `ADE_DB_FILE` env override >
//!   per-platform default (`~/Library/Application Support/ade` on macOS,
//!   `%APPDATA%/ade` on Windows, `$XDG_CONFIG_HOME/ade` on Linux).
//! - Legacy copy: if `ade.db` is missing but a prior-version DB
...
/// Resolves the database file path: explicit arg > `ADE_DB_FILE` > platform
...
pub fn resolve_db_path(db_path: Option<&str>) -> PathBuf {
    ...
    if let Ok(env) = std::env::var("ADE_DB_FILE") {
        let env = env.trim();
        if !env.is_empty() {
...
fn default_db_path() -> PathBuf {
    let data_dir = if cfg!(target_os = "macos") {
        home_dir().join("Library/Application Support/ade")
