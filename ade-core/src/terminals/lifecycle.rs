...
//! (session dedupe), plus the ade-specific env contract from the ticket.
...
/// `ADE_PORT = 50000 + (hash32(portSeed) % 1000) * 10` (ticket env contract).
pub const ADE_PORT_BASE: u16 = 50_000;
...
/// `ADE_PORT = 50000 + (hash32(portSeed) % 1000) * 10` — deterministic,
...
pub fn ade_port(port_seed: &str) -> u16 {
    ADE_PORT_BASE + ((hash32(port_seed) % 1000) * 10) as u16
}

/// The lifecycle env contract (ticket-specified, exact names):
/// `ADE_TASK_ID`, `ADE_TASK_NAME` (slugified, fallback "task"),
/// `ADE_TASK_PATH`, `ADE_ROOT_PATH`, `ADE_DEFAULT_BRANCH` (default "main"),
/// `ADE_PORT` from `ade_port(port_seed)`.
pub fn task_env(
    task_id: &str,
    task_name: &str,
    task_path: &Path,
    ...
    vec![
        ("ADE_TASK_ID".into(), task_id.to_string()),
        ("ADE_TASK_NAME".into(), name),
        (
            "ADE_TASK_PATH".into(),
        ...
        ("ADE_PORT".into(), ade_port(port_seed).to_string()),
    ]
}
