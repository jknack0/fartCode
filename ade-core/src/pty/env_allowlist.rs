...
/// missing) → provider vars → task env (`ADE_*`, E1-06) → hook env.
...
pub fn build_agent_env(
    ...
    if let Some((port, pty_id, nonce, token)) = hook_env {
        env.insert("ADE_HOOK_PORT".into(), port.to_string());
        env.insert("ADE_PTY_ID".into(), pty_id.to_string());
        env.insert("ADE_HOOK_NONCE".into(), nonce.to_string());
        env.insert("ADE_HOOK_TOKEN".into(), token.to_string());
    }
    env
}
...
mod tests {
    ...
    fn base_env_is_always_present() {
        ...
        assert_eq!(env.get("TERM_PROGRAM").map(String::as_str), Some("ade"));
    ...
    fn overlay_order_provider_task_hook() {
        ...
        let env = build_agent_env(
            &[("ADE_PORT".into(), "9999".into())],
            &[
                ("ADE_PORT".into(), "55555".into()),
                ("ADE_TASK_ID".into(), "t1".into()),
            ],
            Some(("8080", "pty-1", "nonce", "tok")),
        );
        assert_eq!(env.get("ADE_PORT").map(String::as_str), Some("55555"));
        assert_eq!(env.get("ADE_TASK_ID").map(String::as_str), Some("t1"));
        assert_eq!(env.get("ADE_HOOK_PORT").map(String::as_str), Some("8080"));
        assert_eq!(env.get("ADE_PTY_ID").map(String::as_str), Some("pty-1"));
        assert_eq!(env.get("ADE_HOOK_TOKEN").map(String::as_str), Some("tok"));
    }
