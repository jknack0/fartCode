...
use ade_providers::ProviderDescriptor;
...
pub fn build_command_with_spill(
    ...
    if !ctx.is_resuming
        && matches!(
            ...
            ade_providers::PromptStrategy::Argv { .. }
        )
    {
...
mod tests {
    ...
    fn claude_fresh_with_prompt_positional() {
        let provider = ade_providers::get("claude").unwrap();
        let mut c = ctx();
    ...
    fn amp_receives_prompt_via_stdin_pipe() {
        let provider = ade_providers::get("amp").unwrap();
        let mut c = ctx();
    ...
    fn codex_resume_without_session_uses_resume_last() {
        let provider = ade_providers::get("codex").unwrap();
        let mut c = ctx();
        c.is_resuming = true;
        c.session_id = Some("conv-1".into());
    ...
    fn letta_fresh_uses_new_conversation_flag() {
        let provider = ade_providers::get("letta").unwrap();
        let mut c = ctx();
        c.session_id = Some("conv-1".into());
    ...
    fn antigravity_injects_session_id_always() {
        let provider = ade_providers::get("antigravity").unwrap();
        let mut c = ctx();
        c.is_resuming = true;
        c.session_id = Some("conv-1".into());
        let cmd = build_command(&c, provider);
    ...
    fn kimi_omits_auto_approve_on_resume() {
        let provider = ade_providers::get("kimi").unwrap();
    ...
    fn spill_writes_and_cleans_temp_file() {
        ...
        assert!(path.starts_with(tmp.path().join(".ade/prompts")));
    ...
    fn spill_composes_with_build_command_for_argv_providers() {
        ...
        let amp = ade_providers::get("amp").unwrap();
