...
//! ticket E1-03).
//!
//! Phase 0 scope: opening a project seeds its settings, excludes `.ade/`
...
/// ade's per-project state directory — excluded from git on project open.
pub const ADE_STATE_DIR: &str = ".ade";

/// Adds `.ade/` to the repo's `.git/info/exclude` (local, per-repo — never a
...
pub fn ensure_ade_git_excluded(git: &dyn GitOps, repo_path: &Path) -> Result<(), Error> {
    let git_dir = git.git_dir(repo_path)?;
    if !git_dir.is_dir() {
        return Ok(());
    }
    let exclude_path = git_dir.join("info").join("exclude");
    let pattern = format!("{ADE_STATE_DIR}/");
...
/// Opens a project (Phase 0): seeds settings, excludes `.ade/` from git,
...
pub fn open_project(
    ...
    ensure_ade_git_excluded(git, &project.path)?;
