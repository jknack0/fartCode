...
//!
//! **Placement note (deviation from §6.4):** the trait lives in `ade-core`
//! (not `ade-git`) so that `ade-core` domain modules (`projects`, later
//! `workspaces`) can depend on it without violating the crate-graph rule that
//! `ade-core` is the leaf (depends on nothing internal). The concrete
//! implementation (`ade_git::CliGit`) lives in `ade-git`, which depends on
//! `ade-core` and re-exports the trait.
//!
...
/// Low-level git operations. Phase 0 implementation: `ade_git::CliGit`
/// (git CLI, no shell).
