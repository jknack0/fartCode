...
//!    deep-merged reads.
//! 2. **Project settings** (`project_settings` table + repo `.ade.json`):
//!    base (DB-backed) fields and a shareable subset; effective precedence
//!    is `defaults < .ade.json < DB-shareable` (later source wins, matching
...
//!    moves local values into `.ade.json` and clears them from the DB.
//!
...
pub trait SettingsStore: Send + Sync {
    ...
    /// next read re-seeds defaults; `.ade.json` is untouched).
...
impl DbSettingsStore {
    ...
    /// default preserve patterns — unless the repo `.ade.json` already
    /// defines `preservePatterns`, in which case the shareable blob starts
    /// empty so the file wins. Called by project creation (E1-03).
    ///
    /// Note: when no `.ade.json` exists at seed time, the default patterns
    /// are materialized into the DB shareable blob, which then beats a
    /// later-created `.ade.json` until the local value is cleared — parity
    ...
    /// as `merge(defaults, .ade.json, DB-shareable)`.
    ...
    /// `shareable_project_settings_json` (which override `.ade.json`).
    ...
    /// needs `ade-git` (see note in code).
    ///
    ...
    /// Reads and parses the repo `.ade.json` (shareable schema). Missing or
        ...
        match serde_json::from_str(&content) {
            ...
            Err(e) => {
                tracing::warn!(
                    ...
                    error = %e,
                    "unparseable .ade.json — falling back to project settings"
                );
                None
            }
        }
...
/// Writes the defined shareable fields into a `.ade.json` config object.
