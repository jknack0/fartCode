...
mod tests {
    ...
    fn accepts_absolute_paths() {
        assert!(normalize_worktree_directory("/ade/worktrees", None).is_ok());
        assert!(
            normalize_worktree_directory(" /ade/worktrees ", None).is_ok(),
            "trimmed"
        );
    }
    ...
    fn expands_tilde() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/Users/ade/worktrees")
        );
        assert_eq!(
            normalize_worktree_directory("~", Some(home)).unwrap(),
            PathBuf::from("/Users/ade")
        );
    ...
    fn windows_absolute_helpers() {
        ...
        assert!(is_windows_drive_absolute("C:\\ade"));
        assert!(is_windows_drive_absolute("C:/ade"));
        assert!(!is_windows_drive_absolute("/ade"));
        assert!(is_windows_unc_absolute("\\\\server\\share"));
        assert!(!is_windows_unc_absolute("//server/share"));
        assert!(is_native_absolute("C:\\ade", true));
        assert!(is_native_absolute("\\\\s\\x", true));
        assert!(is_native_absolute("/ade", false));
