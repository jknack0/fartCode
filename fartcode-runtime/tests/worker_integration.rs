...
//!
//! SessionHost drives the real `fartcode-acp-runtime` binary, which spawns the
...
use fartcode_runtime::protocol::{prepare_session, EnvPair, SessionInput};
use fartcode_runtime::session_host::{PermissionOutcome, SessionHost};

fn worker_bin() -> PathBuf {
    env!("CARGO_BIN_EXE_fartcode-acp-runtime").into()
}

fn adapter_bin() -> PathBuf {
    // The fake adapter is fartcode-acp's bin; it lives in the shared target dir
    // next to this test binary's parent (`target/debug`). Workspace-wide
    // `cargo test` builds it; for isolated runs build fartcode-acp first.
    ...
    assert!(
        path.exists(),
        "fake_acp_adapter not built — run `cargo build -p fartcode-acp --bins` (workspace `cargo test` builds it)"
    );
    path
}
...
async fn renderer_env_discarded_and_server_env_reaches_adapter() {
    ...
    let prepared = prepare_session(
        input,
        &[("FARTCODE_TEST_ENV".into(), "server-resolved".into())],
    )
    .unwrap();
