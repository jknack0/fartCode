# PRD — fartCode, an Agentic Development Environment (Rust + Tauri)

**Status:** v0.3 (architecture decisions + expanded Phase 0 tickets) · **Last updated:** after adding [`ARCHITECTURE.md`](./ARCHITECTURE.md)
**Spawnable tickets:** Phase 0 (E0 + E1 + E2 + E3 subset + E14-01) tracked as GitHub issues — `gh issue list -R jknack0/fartCode --label phase:0`
...
|---|---|---|
| `src/main/` (Electron main) | RPC controllers + domain services + DB + PTY + SSH + updater | `fartcode-app` Tauri backend |
...
| `src/shared/` | Provider registry, IPC primitives, MCP/skills types, events, telemetry | `fartcode-core` shared types + `src/shared` equivalent |
| `src/main/core/*` (45 domains) | One dir per domain, each with `controller.ts` + services | `fartcode-core` modules (§5.6) |
| `src/main/db/` | Drizzle schema + migrations (`drizzle/0000..0019`), `emdash4.db` | `fartcode-core::db` (rusqlite, migration runner) |
| `src/main/core/pty/` | `local-pty` (node-pty) / `ssh2-pty` / tmux / env allowlist | `fartcode-terminal` (portable-pty) |
| `src/main/core/ssh/` | SSH connection mgmt, config parse, client proxy | `fartcode-ssh` (russh) |
| `src/main/core/acp/` | Out-of-process ACP worker host (local + SSH transports) | `fartcode-acp` (new crate) |
| `src/main/core/agent-hooks/` | HTTP hook server, hook config writer, notifications | `fartcode-core::agent_hooks` |
| `src/main/core/dependencies/` | Agent CLI detection/install/update (host deps) | `fartcode-core::dependencies` |
| `src/main/core/automations/` | Scheduler, runs, run transitions | `fartcode-scheduler` |
| `src/main/core/projects/` (+ `worktrees/`, `settings/`) | Project provider pattern, worktree service | `fartcode-core::projects` |
| `src/main/core/tasks/`, `conversations/`, `terminals/` | Task lifecycle, session supervisor, lifecycle scripts | `fartcode-core::tasks/conversations/terminals` |
| `src/main/core/git/`, `github/`, `pull-requests/` | Git ops, GitHub API (gh CLI), PR sync engine | `fartcode-git` + `fartcode-core::github/pr` |
| `src/main/core/integrations/`, `issues/`, `linear/`, `jira/` | Issue-tracker integrations, issue provider registry | `fartcode-integrations` |
| `src/main/core/mcp/`, `skills/`, `prompt-library/` | MCP config sync, skills catalog+install, prompt KV | `fartcode-core::mcp/skills/prompts` |
| `src/main/core/search/`, `editor/`, `view-state/`, `resource-monitor/` | FTS search, editor buffer drafts, view-state KV, resource sampler | `fartcode-core::search/editor/view_state/resource_monitor` |
| `src/main/core/browser/`, `preview-servers/`, `port-forwards/` | Webview browser, dev-server URL detection, SSH port forwards | `fartcode-core::browser/preview/port_forwards` |
| `src/main/core/secrets/`, `account/`, `provider-accounts/`, `shared/oauth-flow` | Encrypted secrets, account + provider token registry, OAuth | `fartcode-core::secrets/account` (keyring) |
| `src/main/core/fs-watch/`, `files/`, `workspaces/` | File watcher worker, file tree, workspace bootstrap | `fartcode-core::fs/files/workspaces` (notify) |
| `apps/workspace-server/` + `packages/core/src/workspace-server/` | Remote daemon exposing git/files/deps/ACP over `@emdash/wire` (SSH-forwarded socket) | `fartcode-server` (new bin crate) |
| `packages/core/src/acp/` | Transport-agnostic ACP client, transcript reducer, session machine | `fartcode-acp` |
| `packages/plugins/src/agents/` | Provider registry: 35 agents, capability descriptors | `fartcode-providers` (Rust registry) |
| `packages/chat-ui/` (Solid) | Chat transcript renderer | `app-frontend` chat components |
| `packages/ui/`, `packages/shared/`, `packages/wire/`, `packages/runtime/` | UI kit, shared primitives, live-model wire, out-of-process runtimes | `app-frontend` + `fartcode-runtime` workers |
...
5. **Workspace-server wire protocol** — semver `PROTOCOL_VERSION`, `initialize` handshake on every connect, `min(clientMinor, serverMinor)` feature negotiation, `PROTOCOL_INCOMPATIBLE` with `upgrade-client|upgrfartcode-server` action. Language-agnostic; port as-is (§5.7).
...
└────────────────────────────────────────────────────────────┘
  │ ACP stdio worker (fartcode-acp-runtime, child process)
  │ SSH-forwarded Unix socket → fartcode-server daemon (remote)
```
...
|---|---|---|
| `fartcode-core` | Domain modules: projects, tasks, conversations, workspaces, settings, library, automations, search, editor buffers, view state, resource monitor, secrets | `src/main/core/*` |
| `fartcode-git` | Git (git2): worktrees, status, staging, diff, commit, push; GitHub/GitLab API clients; PR sync | `src/main/core/git` |
| `fartcode-providers` | Provider registry (35 agents) + capability descriptors + detection/install descriptors | `packages/plugins/src/agents` |
| `fartcode-acp` | ACP client: protocol, session manager/cell, transcript reducer, per-provider adapters | `packages/core/src/acp` + `src/main/core/acp` |
| `fartcode-terminal` | PTY (portable-pty), terminal state persistence, tmux manager | `src/main/core/pty` |
| `fartcode-ssh` | SSH/SFTP (russh), connection profiles, config parse (`ssh -G`), proxy, port forwards | `src/main/core/ssh` + `port-forwards` |
| `fartcode-scheduler` | Cron scheduler, automation state machine, restart recovery | `src/main/core/automations` |
| `fartcode-integrations` | Issue trackers (12), GitHub accounts, CI checks | `src/main/core/integrations|issues|github` |
| `fartcode-telemetry` | Allowlisted event pipeline, feature flags | `src/main/core/telemetry` |
| `fartcode-server` | Remote daemon + wire protocol (JSON-RPC over SSH-forwarded socket) | `apps/workspace-server` + `packages/core/src/workspace-server` |
| `fartcode-runtime` | Out-of-process workers (ACP agent runtime, agent-config resolver, fs-watch) | `packages/runtime` |
| `fartcode-app` | Tauri shell: command modules, events, window, menu, updater | `src/main/index.ts|rpc.ts` + `updates` |
...
1. **SQLite** (`fartCode.db`, mirror their `emdash4.db`): full schema in §6. Drizzle-style numbered migrations + sha256 journal.
2. **`.fartCode.json`** (shareable): only `preservePatterns`, `shellSetup`, `scripts.{setup,run,teardown}`. Local settings override; "Share with team" moves values out of local config.
...
```
FARTCODE_TASK_ID · FARTCODE_TASK_NAME (slug) · FARTCODE_TASK_PATH · FARTCODE_ROOT_PATH
FARTCODE_DEFAULT_BRANCH · FARTCODE_PORT (base of 10-port range)
```

Additional env knobs from reference: `FARTCODE_DB_FILE`, `FARTCODE_DISABLE_NATIVE_DB`, `FARTCODE_DISABLE_CLONE_CACHE`, `FARTCODE_DISABLE_PTY`, `TELEMETRY_ENABLED`.
...
- Major mismatch → `PROTOCOL_INCOMPATIBLE { action: 'upgrade-client'|'upgrfartcode-server', ... }`.
- Same-major = compatible. Never repurpose fields; add-new-and-deprecate-old; unknown JSON keys are ignored (tolerant reader).

Our `fartcode-server` bin crate implements this contract in Rust; negotiation rules ported unchanged.
...
DB file `fartCode.db`; numbered migrations `0000..0019`-style; FTS tables version-gated via `kv` keys (`fts_version`, `file_index_version`).
...
**Features:** Add project (local / clone GitHub / remote SSH) · left sidebar tree with pinned tasks · Project Settings (GitHub account, worktree directory, default branch, base remote, push remote, tmux, workspace provider, preserve patterns, shell setup, lifecycle scripts setup/run/teardown) · `.fartCode.json` sharing + precedence + migration · env contract · script logs in terminal drawer (⌘J) · open project in editor (⌘O) · onboarding flow (sign-in/import steps) · command palette over projects/tasks/conversations + resource monitor view.
...
| E1-02 | Settings store with layered precedence (local > `.fartCode.json`) + `kv` store | M | Precedence tests; Share-with-team moves values |
...
| E1-06 | Lifecycle script runner (setup/run/teardown) + env contract + drawer logs | L | `FARTCODE_*` vars; 10-port range; logs in drawer |
| E1-07 | Preserve-pattern copying into new tasks | S | Never copies tracked files or `.fartCode.json` |
...
**Features:** Add Task (⌘N) from branch/issue/PR · auto/manual name (human-id style names; branch = prefix `fartCode` + random suffix, prefix configurable) · provider + model selector ("Default model") · worktree per task (defaults + override + disable-with-warning) · agent spawn (TUI via PTY or ACP) · terminal state autosave/resume · conversations (⌘T/⌘D/⌘⇧A/⌘Enter) · multiple terminals (⌘⇧T) · delete tasks (⌘Backspace, teardown agents/tmux/worktrees) · task switching (⌘⌥↑/↓) · task status lifecycle + telemetry.
...
| E6-02 | Dev-server URL detection from terminal output + preview lifecycle | M | Ref: `terminal-url-detector.ts`; ports from `FARTCODE_PORT` |
...
**Features:** capabilities (issues/PRs/checks/comments/repo-create) · connection methods: fartCode-account OAuth, **gh CLI import**, device flow · account manager (default account, removal cascade) · per-project selection · GitHub API via own client (ref: their `github/` uses gh CLI + Octokit; we use `reqwest` + GraphQL/REST).
...
**Features:** app-wide default + per-project toggle · deterministic `fartCode-<encoded PTY ID>` sessions · create-if-missing, mouse + large history · reattach on restart/reconnect · scrollback in tmux, output streamed · cleanup on delete · Windows local never tmux.
...
Rust equivalent:
- `cargo test` per crate; `fartcode-core::db` integration tests against real SQLite temp files (their `main-db`).
...
2. **Secrets hygiene**: OS keychain only; never SQLite/`.fartCode.json`/logs; redaction in file logging.
...
7. **Sign-in/account** — reference has optional fartCode account + `provider_accounts`. Recommend skipping our own account system; device flow + gh import cover GitHub.
...
10. **Name/branding** — resolved: `fartCode` (crate prefix `fartCode-*`).
